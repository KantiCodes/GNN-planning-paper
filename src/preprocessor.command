...
def default_gnn_preprocessor(
    threshold, retries, model_path, use_relaxed_plan, use_simple_landmarks
):
    # Original step
    input = sys.stdin
    ...
    # GNN STEP
    run_gnn_preprocessor(
        sas_path=WORKSPACE_SAS,
...
def failed_gnn_preprocessor(failed_count, retries):
    if failed_count == retries or not os.path.exists("workspace/retries"):
        ...
        logger.warning(
            "Retries exceeded or no retries available, using original sas file,\
...
argparser.add_argument(
    "--gnn-threshold", help="number of retries for gnn", type=float, required=True
)
argparser.add_argument(
    "--gnn-retries", help="number of retries for gnn", type=int, required=True
)
argparser.add_argument(
    "--failed",
    help="0 based index, indicating if we have already failed a plan",
    default=-1,
    type=int,
)
