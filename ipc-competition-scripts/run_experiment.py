...
class RunExperiment:
    ...
    def run (self, path_exp, cached_rev, planner, config, ENV, SUITE, build_options = [], driver_options = [], extra_parsers = []):
        ...
        if(planner.split("/")[-1] == "scorpion"):
            exp.add_resource("", PLANNER / "builds" / "release" / "bin", "code/builds/release/bin")
            additional_options = ["--keep-sas-file", "--overall-time-limit", self.time_limit]
        else:
            exp.add_resource("", PLANNER / "builds" / "release64" / "bin", "code/builds/release64/bin")
            additional_options = ["--overall-time-limit", self.time_limit, "--overall-memory-limit", self.memory_limit]

        for task in SUITE:
            algo = _DownwardAlgorithm(
                f"name",
                cached_rev,
                driver_options + additional_options,
                config,
            )
