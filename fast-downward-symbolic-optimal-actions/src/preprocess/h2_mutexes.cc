...
bool compute_h2_mutexes(const vector <Variable *> &variables,
    ...
    cout << "Total mutex and disambiguation time: " << (double)(clock() - start_t) / CLOCKS_PER_SEC << " iterations: " << num_iterations <<  " mutex fw " << total_mutexes_fw <<  " mutex bw " << total_mutexes_bw  << endl;
    return true;
}
