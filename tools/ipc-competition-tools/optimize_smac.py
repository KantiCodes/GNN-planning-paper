...
class Eval:
    ...
    def target_function(self, config: Configuration, instance: str, seed: int) -> float:
        model_settings, target_folder, batch_size = parse_config(config)
        ...
        print(f"Running {instance} with {model_settings} and {target_folder} and batch size: {batch_size}")

        # TODO: setup limits
        model_path = run_step_gnn_learning(self.GNN_REPO_DIR, model_settings, f'{self.GNN_DATA_DIR}/{target_folder}', f'{self.GNN_LEARNING_DIR}/{target_folder}', batch_size=batch_size, time_limit=1200, memory_limit=4*1024*1024)

        if model_path is None:
            return math.inf
            return {"time": math.inf, "operators": math.inf, "cost": math.inf}
        ...
        try:
            ...
            os.makedirs(f"data/smac_runs/{target_folder}/{PROBLEM}/{batch_size}", exist_ok=True)
            with open(f"data/smac_runs/{target_folder}/{PROBLEM}/{batch_size}/run.log", "w") as f:
                f.write(read_output)

            if error_output:
                with open(f"data/smac_runs/{target_folder}/{PROBLEM}/{batch_size}/error.log", "w") as f:
                    f.write(error_output.decode())
            ...
            if is_retries_excited:
                ...
                return 10000000
                return {"time":10000000, "operators":10000000, "cost": 10000000}
...
def parse_config(config):
    ...
    return model_settings, target_operators, batch_size
...
def run_smac(ROOT, DATA_DIR, WORKING_DIR, domain_file, instance_dir, instances_with_features : dict, instances_properties : dict, walltime_limit, n_trials, n_workers, run_id, extra_flags):
    ...
    target_folder = Categorical('target_folder', ["good-operators-unit"], default="good-operators-unit")
    ...
    batch_size = Categorical('batch_size', [4,6,8, 16], default=16)
    lr = UniformFloatHyperparameter('lr', 1e-03, 0.02)

    parameters = [target_folder, layers_num, hidden_size, conv_type, aggr, optimizer, lr, batch_size]
    ...
    model_setting, target_folder, batch_size = parse_config(incumbent)
